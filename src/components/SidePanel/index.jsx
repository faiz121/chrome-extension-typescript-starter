...
const SidePanel = () => {
  ...
  useEffect(() => {
    chrome.storage.local.get(['session'], (result) => {
      ...
      console.log('sessionData.loginTimestamp---------', sessionData?.loginTimestamp);
  ...
  const handleTextareaChange = (e) => {
    ...
    if (newInputValue.length > 3000) {
      ...
      const newHeight = Math.min(Math.max(e.target.scrollHeight, 56), window.innerHeight * 0.25);
  ...
  const getSuggestionsPanelHeight = () => {
    return 40;
  };
  ...
  const handleSuggestionClick = async (suggestion) => {
    ...
    requestAnimationFrame(() => {
      ...
      if (suggestionsPanel && chevron) {
        ...
        // Calculate distance to bottom-left of chevron
        const translateX = chevronRect.left - panelRect.left;
        const translateY = chevronRect.bottom - panelRect.top;

        suggestionsPanel.style.transition = 'all 700ms cubic-bezier(0.4, 0, 0.2, 1)';
        suggestionsPanel.style.transformOrigin = 'left bottom';

        requestAnimationFrame(() => {
          suggestionsPanel.style.transform = `
    ...
    try {
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (response.error) {
            ...
            if (
              response.error === 'Could not establish connection. Receiving end does not exist.'
            ) {
              setSuggestionError(response.error);
              setMessages((prev) =>
                prev.map((msg, idx) =>
                  idx === prev.length - 1
                    ? { ...msg, isLoading: false, content: '', isSuggestion: true }
                    : msg
                )
              );
      ...
      setMessages((prev) =>
        prev.map((msg, idx) => (idx === prev.length - 1 ? { ...msg, isLoading: false } : msg))
      );
      setIsProcessing(false);
    }
  };
  ...
  const toggleSuggestions = () => {
    ...
    if (suggestionsPanelRef.current) {
      if (isSuggestionsVisible) {
        ...
        suggestionsPanelRef.current.style.transition = 'all 0.2s ease-out';
        suggestionsPanelRef.current.style.transform = 'translateY(90%)';
        suggestionsPanelRef.current.style.opacity = '0.95';
      } else {
        // Animate up
        suggestionsPanelRef.current.style.transition = 'all 0.2s ease-out';
        suggestionsPanelRef.current.style.transform = 'translateY(0)';
        suggestionsPanelRef.current.style.opacity = '1';
      }

      setIsAnimatingChevron(true);
      setTimeout(() => setIsAnimatingChevron(false), 200);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className='absolute z-50 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg'>
        ...
        {currentView !== 'incognito' && (
          <button
            onClick={() => {
              setCurrentView('incognito');
              setIsViewDropdownOpen(false);
            }}
            className='w-full px-4 py-2 text-left hover:bg-gray-100 rounded-lg flex items-center gap-2'
          >
        ...
        <div className='flex-1 flex flex-col min-h-0 relative'>
          {currentView === 'ragonfly' ? (
            <>
              <div className='flex-1 overflow-y-auto pb-[250px] scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400 scrollbar-track-transparent'>
                {' '}
                {isLoading ? (
                    ...
                    {messages.length === 0 ? (
                      ...
                      >
                        <div
                          ...
                          style={{ marginTop: 'calc((100vh - 60px - 250px) / 2)' }}
                        >
                      ...
                      <div className='px-4 py-4'>
                        <div className='space-y-4'>
                          ...
                          {suggestionError && (
                            <div className='text-red-500 text-sm'>{suggestionError}</div>
                          )}
              ...
              <div
                ...
                style={{
                  zIndex: 50,
                  height: `${
                    textareaHeight + (isSuggestionsVisible ? getSuggestionsPanelHeight() : 50)
                  }px`,
                  backgroundColor: 'rgb(246, 246, 246)',
                }}
              >
                <div
                  className={`absolute w-full suggestions-panel border-gray-200 transition-all duration-100 ease-in-out origin-center bg-white
                    ${
                      !isFirstInteraction
                  ...
                  style={{
                    ...
                    bottom: `${isSuggestionsVisible ? textareaHeight + 40 : textareaHeight + 65}px`,
                    transform: isSuggestionsVisible ? 'translateY(0)' : 'translateY(100%)',
                  }}
                >
                  ...
                  <div
                    className={`w-full ${!isSuggestionsVisible ? 'opacity-0' : 'opacity-100'}`}
                ...
                >
                  <div
                    className='bg-white shadow-sm rounded-3xl border border-gray-300 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition-all'
                    style={{
                  ...
                  >
                    {/* Add a white background div that extends full height */}
                    <div
                    ...
                    <form onSubmit={handleSubmit} className='flex flex-col pt-8'>
                      <div className='flex items-start'>
                        <textarea
                          ...
                          className='flex-1 px-3 py-3 bg-transparent border-none text-base focus:outline-none leading-normal placeholder:text-sm resize-none overflow-y-auto'
                          style={{
                            minHeight: '56px',
                            maxHeight: '25vh',
                            fontSize: '14px',
                        ...
                        />
                        <div className='flex items-center space-x-1 pr-2 mt-3'>
                          {' '}
                          {/* Add mt-3 to align with input */}
                          <ContextSelector
                            files={files}
                            setFiles={setFiles}
            ...
            <IncognitoMode currentGradient={currentGradient} username={session?.username} />
          ) : null}
        </div>
      </div>
      {isSettingsOpen && (
